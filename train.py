...
def get_parser():
    ...
    parser.add_argument('--arch', type=str, default='tbrnet')  #
    parser.add_argument('--viz', action='store_true', default=True)
    parser.add_argument('--config', type=str, default='config/iSAID/iSAID_split0_resnet50.yaml',
...
def main():
    ...
    if args.data_set == 'pascal' or args.data_set == 'coco' or args.data_set == 'iSAID' or args.data_set == 'DLRSD':
...
def train(train_loader, val_loader, model, optimizer, epoch):
    ...
    for i, (input, target, target_b, s_input, s_mask, subcls) in enumerate(train_loader):
        ...
        output, main_loss, aux_loss1, aux_loss2, aux_loss = model(s_x=s_input, s_y=s_mask, x=input, y_m=target, y_b=target_b,
                                                        cat_idx=subcls)
        loss = main_loss + args.aux_weight1 * aux_loss1 + args.aux_weight2 * aux_loss2 + args.aux_weight1 * aux_loss # 一个数
        ...
        if (i + 1) % args.print_freq == 0 and main_process():
            ...
            if args.viz:
                ...
                writer.add_scalar('loss_train_aux', aux_loss_meter.val, current_iter)
...
def validate(val_loader, model):
    ...
    for e in range(db_epoch):
        for i, (input, target, target_b, s_input, s_mask, subcls, ori_label, ori_label_b,class_id) in enumerate(val_loader):
            ...
            if args.ori_resize:  # 真值转化为方形
                longerside = max(ori_label.size(1), ori_label.size(2)) # ori_label: 1,512,512
                backmask = torch.ones(ori_label.size(0), longerside, longerside, device='cuda') * 255   # bs,256,256  全是255
    ...
    class_miou_b = class_miou_b * 1.0 / (
                len(class_intersection_meter_b) - len(target_b[target_b == 0]))

    if main_process():
        logger.info('meanIoU---Val result: mIoU_f {:.4f}.'.format(class_miou))  # final
        logger.info('meanIoU---Val result: mIoU_m {:.4f}.'.format(class_miou_m))
        logger.info('meanIoU---Val result: mIoU_b {:.4f}.'.format(class_miou_b))

        logger.info('<<<<<<< Novel Results <<<<<<<')
        for i in range(split_gap):  # 0,1,2,3,4   -> +1   1,2,3,4,5
        ...
        for i in range(split_gap * 3):  # 0-14
